...
public partial class EditorTab : FluxorComponent
{
    [Inject]
    ...
    {
        Dispatcher.Dispatch(
            new EditorState.SetActiveTextEditorKeyAction(TextEditor.Key));
    }
    ...
    private void FireDisposePlainTextEditorOnClick()
    {
        ...
        if (localTextEditors.Count - 1 == 0)
            Dispatcher.Dispatch(new EditorState.SetActiveTextEditorKeyAction(TextEditorKey.Empty));
        else
        {
            var nextActiveTextEditor = localTextEditors[indexOfNextActiveTextEditor];

            Dispatcher.Dispatch(new EditorState.SetActiveTextEditorKeyAction(nextActiveTextEditor.Key));
        }
